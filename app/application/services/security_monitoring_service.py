...
from domain.repositories import LoginAttemptsRepositoryInterface
...
class SecurityMonitoringService:
    ...
    async def process_create_media(self, user_id : UUID, request_time : datetime.datetime) -> None:
    ...
    async def record_create_media_attempt(self, user_id : UUID, request_time : datetime.datetime) -> None:
        
        entity = await self._create_media_attemp_from_dto(user_id, request_time)

        async with self._uow:
            create_media_attempt_repository = await self._uow.get_create_media_attempt_repository()
            await create_media_attempt_repository.insert(entity)
    
    async def record_login_attemp(self, ip : str, request_time : datetime.datetime) -> None:
    ...
    async def _user_is_blocked(self, user_id : UUID) -> bool:
        ...
        async with self._uow:
            ...
            banned_users = await create_media_attempt_repository.get_user_ids_with_excessive_attempts(
    ...
    async def _create_login_attemp_from_dto(self, ip : str, request_time : datetime.datetime) -> LoginAttempt:
        return LoginAttempt(
            ip_address=ip,
            timestamp=request_time,
        )
        
    async def _create_media_attemp_from_dto(self, user_id : UUID, request_time : datetime.datetime) -> CreateMediaAttempt:
