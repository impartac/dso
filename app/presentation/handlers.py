...
from .dependencies import check_rate_limit, get_current_user, get_services, get_uow, mask_pii
from .models import (
    ErrorResponse,
    LoginRequest,
    MediaCreateRequest,
...
async def create_media(
    ...
    if not check_rate_limit(user_key, limit=10, window=60):
        logger.warning(f"Rate limit triggered for user {current_user.id} - rule_id: media_create")
        raise HTTPException(
    ...
    try:
        ...
        if not user:
            ...
            try:
                ...
                logger.warning(f"Login rate limit exceeded for IP {client_ip} - Correlation ID: {correlation_id}")
                raise HTTPException(
                    status_code=status.HTTP_429_TOO_MANY_REQUESTS,
            ...
            logger.warning(f"Failed login attempt: user not found - Email: {mask_pii(login_data.email)} - Correlation ID: {correlation_id}")
            raise HTTPException(
        ...
        if not verify_password(login_data.password, user.hash_password):
            try:
                ...
                retry_seconds = int((e.retry_after - datetime.utcnow()).total_seconds())
                logger.warning(f"Login rate limit exceeded for user {user.id} - Correlation ID: {correlation_id}")
                raise HTTPException(
                    status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                    detail="Too many login attempts. Please try again later.",
                    headers={"Retry-After": str(retry_seconds)}
            ...
            logger.warning(f"Failed login attempt: invalid password - User ID: {user.id} - Correlation ID: {correlation_id}")
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail="Invalid email or password"
            )
        ...
        try:
            ...
            retry_seconds = int((e.retry_after - datetime.utcnow()).total_seconds())
            logger.warning(f"Login rate limit exceeded during successful login - User ID: {user.id} - Correlation ID: {correlation_id}")
            raise HTTPException(
                status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                detail="Too many login attempts. Please try again later.",
                headers={"Retry-After": str(retry_seconds)}
        ...
        logger.info(f"Successful login: User ID: {user.id} - Correlation ID: {correlation_id}")
        ...
        await uow.rollback()
        logger.error(f"Unexpected error during login: {e} - Correlation ID: {correlation_id}")
        raise HTTPException(
...
async def register_user(
    ...
    try:
        async with uow as uow:
            ...
            if existing_user:
                logger.warning(f"Registration attempt with existing email: {user_data.email} - Correlation ID: {correlation_id}")
                raise HTTPException(
            ...
            if not check_rate_limit(ip_key, limit=3, window=3600):  # 3 попытки в час с одного домена
                logger.warning(f"Registration rate limit exceeded for domain: {user_data.email} - Correlation ID: {correlation_id}")
                raise HTTPException(
                    status_code=status.HTTP_429_TOO_MANY_REQUESTS,
            ...
            logger.info(f"User registered successfully: {user_data.email} - User ID: {user.id} - Correlation ID: {correlation_id}")
        ...
        await uow.rollback()
        logger.error(e)
        logger.error(f"Error during user registration: {e} - Correlation ID: {correlation_id}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
