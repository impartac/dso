...
from domain.errors import CreateMediaRateLimitException
...
async def create_media(
    ...
    security_monitoring_service: Annotated[
        SecurityMonitoringService, Depends(get_security_monitoring_service)
    ],
) -> MediaResponse:
    ...
    try:
        await security_monitoring_service.process_create_media(
            current_user.id, request_time
        )
        logger.debug(
            f"Successful process create_media reuqest. user = {current_user.id}, \
                request_time = {request_time}"
        )
    except CreateMediaRateLimitException as cm_rt:
        raise HTTPException(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS,
            detail={"message": "To many request"},
            headers={"Retry-After": str(cm_rt.blocked_until)},
        )
...
async def get_media(
    ...
    if not await authorization_service.can_get_media(current_user.id, media.owner_id):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail={"message": "No permission"}
        )
